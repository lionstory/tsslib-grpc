...
func (p *Public) MarshalString() (string,error){
	n := p.N.Nat().Bytes()
	data := fmt.Sprintf("%s|%s|%s", hex.EncodeToString(n), hex.EncodeToString(p.S.Bytes()),
		hex.EncodeToString(p.T.Bytes()))
	return data, nil
}

func (p *Public) UnmarshalString(data string) error{
	parts := strings.Split(data, "|")
	n, err := hex.DecodeString(parts[0])
	if err != nil{
		return err
	}
	p.N = safenum.ModulusFromBytes(n)
	s, err := hex.DecodeString(parts[1])
	if err != nil{
		return err
	}
	p.S.SetBytes(s)
	t, err := hex.DecodeString(parts[2])
	if err != nil{
		return err
	}
	p.T.SetBytes(t)
